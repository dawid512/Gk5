...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
    ...
    public void Korkociag(GL2 gl2,int radius, int numberOfTurns ){
        ...
        int numVerticles = numberOfTurns*1000;
        double angleIncrement = numberOfTurns * Math.PI /numVerticles;
        float height =-8;

        gl2.glBegin(GL2.GL_POLYGON);
        for (int i = 1; i <= numVerticles; i++){
    ...
    public void Polygon(GL2 gl2, int numVerticles){
        gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);

        gl2.glBegin(GL2.GL_POLYGON);
        double angleIncrement = 2 * Math.PI /numVerticles;
        for (int i = 1; i <= numVerticles; i++) {
            gl2.glColor3f(x / 0.6f, (x + z) * x / 0.4f, z / 0.5f);
            x = (float) (radius * Math.cos(angle));
            z = (float) (radius * Math.sin(angle));
    ...
    public void PiramidShape(GL2 gl2, double size){
        gl2.glBegin(GL2.GL_TRIANGLE_FAN);
        int n = 15; // liczba wierzchołków
        gl2.glVertex3f(0,4,4);
